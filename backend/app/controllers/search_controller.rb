class SearchController < ApplicationController
  def search 
    ...
    if flight_param[:flight_number].length != 0
      ...
      url = "https://aviation-edge.com/v2/public/flights?key=#{ENV['AVIATION_API_KEY']}&lat=#{flight_param[:lat]}&lng=#{flight_param[:lng]}&distance=#{flight_param[:distance]}&limit=300"
    end
